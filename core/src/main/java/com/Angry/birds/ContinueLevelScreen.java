...
public class ContinueLevelScreen implements Screen {
    ...
    public ContinueLevelScreen(Game game) {
        this.game = game;
    ...
    public void show() {
        ...
        blocks.add(new Block(blockTexture1, world, 680, 70, 24, 206, 0f,2));
        ...
        pauseButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                game.setScreen(new PauseScreen(game, ContinueLevelScreen.this));
            }
        });
    ...
    private void handleInput() {
        if (Gdx.input.isTouched() && !isBirdLaunched) {
            if (!isDragging) {
                isDragging = true;
                initialPosition = new Vector2(bird.getX(), bird.getY()); // Initial bird position
                bird.setStatic(true); // Keep bird static while dragging
            }
    ...
    @Override
    public void hide() {
        System.out.println("ContinueLevelScreen hidden");
    }
