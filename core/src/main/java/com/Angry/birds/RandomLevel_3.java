...
public class RandomLevel_3 implements Screen {
    ...
    public RandomLevel_3(Game game) {
        this.game = game;
        world = new World(new Vector2(0, -5f), true);
        birds = new ArrayList<>();
        for (int i = 0; i < 3; i++) { // Add 3 birds for example
            birds.add(new Bird(new Texture(Gdx.files.internal("Red.png")), world));
        }
        bird = birds.get(currentBirdIndex); // Start with the first bird
    ...
    public void show() {
        ...
        blockTexture2 = new Texture(Gdx.files.internal("concrete.png"));
        ...
        pauseButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                game.setScreen(new PauseScreen(game, RandomLevel_3.this));
            }
        });
    ...
    private void Bird_performed_what_happening() {
        // Check bird collisions with pigs
        for (Pig pig : pigs) {
